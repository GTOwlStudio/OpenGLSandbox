...
{
public:
	gui_devtest(std::string text, FontAtlas &atlas, glm::vec4 xysxsy, float depth,float  r, float g, float b);
	gui_devtest(std::string text, FontAtlas &atlas, float x, float y, float depth,float  r, float g, float b);
	//gui_devtest(std::string text, FontAtlas &font, float x, float y, float depth, LABEL_POS_MODE  posmode);
	~gui_devtest();
	void load();
	void update();
