...
engine_shader::engine_shader(): m_vertexID(0), m_fragmentID(0), m_programID(0), m_vertexSource(), m_fragmentSource()
{
	...
	std::cout<< "engine_shader constructor call" << std::endl;
}
...
{
	std::cout<< "engine_shader constructor called" << std::endl;
}
...
bool engine_shader::load()
{
	...
	//int a = 0;
//	GLboolean a = glIsShader(m_vertexID);
...
bool engine_shader::compileShader(GLuint &shader, GLenum type, std::string const &fichierSource)
{
    // Création du shader
